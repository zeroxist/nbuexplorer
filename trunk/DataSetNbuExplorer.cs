...
namespace NbuExplorer
{
    ...
    public partial class DataSetNbuExplorer
    {
        ...
        private static string storeFileName = Path.Combine(Preferences.AppSettingsFolder, "Data.xml");
        ...
        public static void AddMessageFromVmg(Vcard vmg)
        {
            ...
            MessageRow row = _defaultInstance.Message.AddMessageRow(vmg.MessageBox, vmg.MessageTime, number, name, vmg.MessageBody, hash, vmg.MessageBody.Length);
        ...
        private static void AddMessageInternal(Message m)
        {
            ...
            MessageRow row = _defaultInstance.Message.AddMessageRow(m.DirectionBox,
                         ...
                         m.MessageText,
                         hash,
                         m.MessageText.Length);
            if (row.time == DateTime.MinValue) row.SettimeNull();
        }

        public static void Load()
        {
            if (Preferences.Instance.SaveDataOnExit
                && File.Exists(storeFileName))
            {
                try
                {
                    _defaultInstance.ReadXml(storeFileName);
                    return;
                }
                catch
                {
        ...
        public static void Save()
        {
            ...
            try
            {
                if (File.Exists(storeFileName))
                {
                    try
                    {
                        string backupFile = Path.ChangeExtension(storeFileName, ".backup");
                        File.Delete(backupFile);
                        File.Move(storeFileName, backupFile);
                ...
                _defaultInstance.WriteXml(storeFileName);
            }
            catch
            {
            }
