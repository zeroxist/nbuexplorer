...
namespace NbuExplorer
{
    public partial class FormMain : Form
    {
            ...
            using (FileStream fs = File.OpenRead(currentFileName))
            {
                ...
                else
                {
                    ...
                    while ((current = fs.ReadByte()) > -1)
                    {
                        ...
                        #region jpeg
                        if (Preferences.Instance.BruteForce.Jpg && Pattern.Jpeg.Step((byte)current, fs.Position))
                        {
                            current = fs.ReadByte();
                        ...
                        #region media
                        if (Preferences.Instance.BruteForce.Mp4 && Pattern.Media.Step((byte)current, fs.Position))
                        {
                        ...
                        #region pkzip
                        if (Preferences.Instance.BruteForce.Zip && Pattern.PkZip.Step((byte)current, fs.Position))
                        {
                            fs.Seek(4, SeekOrigin.Current);
                        ...
                        #region vcards
                        if (Preferences.Instance.BruteForce.Vcards)
                        {
            ...
            while ((ze = zi.GetNextEntry()) != null)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (Preferences.Instance.MessageSources.Predef && Message.MsgFileNameRegex.IsMatch(item.Filename))
                        {
        ...
        private void addSymbianFile(string root, string dir, FileInfo fi)
        {
            ...
            else if (Preferences.Instance.MessageSources.Symbian && ExpectMessage(dir))
            {
                    ...
                    for (int j = 0; j < count; j++)
                    {
                        ...
                        else
                        {
                            ...
                            try
                            {
                                ...
                                if (Preferences.Instance.MessageSources.Vmg)
                                {
                ...
                default:
                    ...
                    if (procAsDefault)
                    {
                        ...
                        for (int j = 0; j < count; j++)
                        {
                            ...
                            if (Preferences.Instance.MessageSources.Predef && Message.MsgFileNameRegex.IsMatch(filename))
                            {
                                try
                                {
                                    using (MemoryStream ms = fi.GetAsMemoryStream())
                    ...
                    if (countSms > 0)
                    {
                        ...
                        for (int j = 1; j <= countSms; j++)
                        {
                            ...
                            if (!ms.IsDelivery)
                            {
                                ...
                                if (Preferences.Instance.MessageSources.Binary)
                                {
